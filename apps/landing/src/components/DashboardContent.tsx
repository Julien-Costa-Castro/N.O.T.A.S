...
export default function DashboardContent({ className = "", step, setStep }: DashboardContentProps) {
  ...
  return (
    <div className={`w-full h-full bg-white select-none text-left relative overflow-hidden ${className}`}>
      ...
      {/* Étape 0 : Écran d'accueil sombre d'onboarding (rendered statically outside zoom) */}
      {currentStep === 0 && (
      ...
      {/* Zooming and panning dashboard container inside the laptop screen */}
      <motion.div
        animate={{
        ...
        className="flex w-full h-full bg-white relative overflow-hidden"
      >
        ...
        <aside className="hidden md:flex w-44 shrink-0 flex-col border-r border-gray-100 bg-[#FBFBFA] p-3">
          ...
          <nav className="space-y-0.5 font-sans">
            ...
            <div className="flex items-center justify-between px-2 py-1.5 rounded-md text-[10px] text-neutral-500">
              <span className="flex items-center gap-2"><Send className="w-3.5 h-3.5" /> Relances</span>
              <span className="text-[7px] font-mono text-emerald-700 bg-emerald-50 border border-emerald-100 rounded px-1">12</span>
            </div>
            <div className="flex items-center justify-between px-2 py-1.5 rounded-md text-[10px] text-neutral-500">
              <span className="flex items-center gap-2"><Clock className="w-3.5 h-3.5" /> Alertes</span>
